...
abstract contract ERC20Interface {
    ...
    event Callvoting_(address wallet, uint votingId, uint256 CalledTokenRate);
    event Vote_(address wallet, uint votingId, bool Decision);
    event Stopvoting_(uint256 votingId, uint256 possible_price_, bool decision);
}
...
contract erc20RedDuck is ERC20Interface {
    ...
    function callvoting(uint new_price) public returns (bool success) {
        ...
        emit Callvoting_(msg.sender, voting_id, possible_price);
    ...
    function vote(bool decision) public returns (bool success){
        ...
        emit Vote_(msg.sender, voting_id, decision);
    ...
    function stopvoting() public returns (bool success){  //эту функцию вызываем мы сами с помощью ether.js ровно через 50 минут после начала голосования
        ...
        emit Stopvoting_(voting_id, possible_price, result); // записали в блокчейн, чем это закончилось
        return true;
    }
